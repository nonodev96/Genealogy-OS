...
export class ExportService {
	...
	exportSVG(tree: FamilyTree, svgElement?: SVGSVGElement): string {
		if (svgElement) {
			...
			const liveG = svgElement.querySelector(
				"g.zoom-layer",
			) as SVGGElement | null;
			let vx = 0,
				vy = 0,
	...
	private buildSVGFromScratch(tree: FamilyTree): string {
		...
		layout.edges.forEach((e) => {
			...
			lines.push(
				`<text x="${mx}" y="${my - 4}" font-size="9" fill="#888" text-anchor="middle">${TreeLayoutService.label(e.type)}</text>`,
			);
		});
		...
		layout.nodes.forEach((n) => {
			...
			if (n.person.birthDate) {
				lines.push(
					`<text class="node-date" x="${x + NODE_W / 2}" y="${y + 48}" text-anchor="middle">🎂 ${n.person.birthDate}</text>`,
				);
			}
		});
	...
	exportText(tree: FamilyTree): string {
		...
		tree.persons.forEach((person) => {
			...
			const rels = tree.relations.filter(
