...
import {
  ...
  PARENT_TYPES, CHILD_TYPES, PARTNER_TYPES, SIBLING_TYPES,
...
export class TreeLayoutService {
  ...
  computeLayout(persons: Person[], relations: Relation[]): TreeLayout {
    if (persons.length === 0) {
      return { nodes: new Map(), edges: [], width: 0, height: 0, levelCount: 0 };
    }
    ...
    for (const rel of relations) {
      this.addRelation(rel.from, rel.to, rel.type, parentOf, childOf, partnerOf, siblingOf);
    }
    ...
    const positions = this.assignPositions(byLevel, partnerOf, childOf, parentOf);
  ...
  private assignLevels(
    ...
    const queue: { id: string; lvl: number }[] = startNodes.map(id => ({ id, lvl: 0 }));
  ...
  private buildPartnerGroups(ids: string[], partnerOf: Map<string, Set<string>>): string[][] {
    const groups: string[][] = [];
  ...
  static edgeColor(type: RelationType): string {
    if (PARENT_TYPES.includes(type) || CHILD_TYPES.includes(type)) return '#4f46e5'; // indigo
  ...
  static isDashed(type: RelationType): boolean {
    return ['adoptiveParentOf','adoptiveChildOf','stepParentOf','stepChildOf',
            'guardianOf','wardOf','ancestorOf','descendantOf'].includes(type);
  }
...
export { NODE_W, NODE_H, H_GAP, V_GAP };