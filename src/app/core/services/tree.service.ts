...
export class TreeService {
	...
	async addRelation(
		...
		const duplicate = tree.relations.find(
		...
		await this.storage.saveTree({
			...tree,
			relations: [...tree.relations, relation],
		});
		return relation;
	}
	...
	async updateRelation(treeId: string, updated: Relation): Promise<void> {
		...
		if (!tree) return;
		this.history.snapshot(tree);
		const relations = tree.relations.map((r) =>
	...
	getRelatedPersons(
		...
		return this.getRelationsForPerson(tree, personId)
			.map((rel) => {
